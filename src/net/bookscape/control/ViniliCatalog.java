 ...
 */
@WebServlet("/ViniliCatalog")
public class ViniliCatalog extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public ViniliCatalog() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		vinili = musics.stream().filter(v -> v.getFormato().toString().equalsIgnoreCase("vinile")).collect(Collectors.toList());
		
		request.setAttribute("musics", vinili);
		request.setAttribute("valutazioni", valutazioni);
		
		RequestDispatcher dispatcher = request.getRequestDispatcher("viniliCatalog.jsp");
